...
const testimonials = [
  ...
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1639149888905-fb39731f2e6c?w=900&auto=format&fit=crop&q=60",
  },
  {
    quote:
    ...
    image:
      "https://plus.unsplash.com/premium_photo-1689977830819-d00b3a9b7363?w=900&auto=format&fit=crop&q=60",
  },
]

export default function Home() {
  return (
    <main className="min-h-screen flex items-center justify-center bg-black p-8 w-full">
      <TestimonialsMinimal />
    </main>
  )
}
      ...
      <div className="relative min-h-[80px] mb-12">
        {testimonials.map((t, i) => (
          <p
            key={i}
            className={`
              absolute inset-0 text-xl md:text-2xl font-light leading-relaxed
              transition-all duration-500 ease-out
              ${
                active === i
                  ? "opacity-100 translate-y-0 blur-0 text-white"
                  : "opacity-0 translate-y-4 blur-sm pointer-events-none text-white"
              }
            `}
          >
            "{t.quote}"
        ...
        <div className="flex -space-x-2">
          {testimonials.map((t, i) => (
            <button
              ...
              onClick={() => setActive(i)}
              className={`
                relative w-10 h-10 rounded-full overflow-hidden ring-2 ring-black
